...
// Unix
#if (                                                                                                                                                \
    (ELPP_OS_LINUX || ELPP_OS_MAC || ELPP_OS_FREEBSD || ELPP_OS_NETBSD || ELPP_OS_SOLARIS || ELPP_OS_AIX || ELPP_OS_EMSCRIPTEN || ELPP_OS_QNX)       \
    && (!ELPP_OS_WINDOWS))
...
#define ELPP_ASSERT(expr, msg)                                                                                                                       \
    if (!(expr)) {                                                                                                                                   \
        ...
        ELPP_INTERNAL_DEBUGGING_OUT_ERROR << "EASYLOGGING++ ASSERTION FAILED (LINE: " << __LINE__ << ") [" #expr << "] WITH MESSAGE \""              \
...
#define ELPP_ASSERT(expr, msg)                                                                                                                       \
    if (!(expr)) {                                                                                                                                   \
        std::stringstream internalInfoStream;                                                                                                        \
        internalInfoStream << msg;                                                                                                                   \
        ELPP_INTERNAL_DEBUGGING_OUT_ERROR << "ASSERTION FAILURE FROM EASYLOGGING++ (LINE: " << __LINE__ << ") [" #expr << "] WITH MESSAGE \""        \
                                          << ELPP_INTERNAL_DEBUGGING_MSG(internalInfoStream.str()) << "\"" << ELPP_INTERNAL_DEBUGGING_ENDL;          \
    }
...
#else
#if (                                                                                                                                                \
    (ELPP_COMPILER_CLANG && defined(ELPP_CLANG_SUPPORTS_THREAD)) || (!ELPP_COMPILER_CLANG && defined(ELPP_CXX11))                                    \
...
public:
    ...
    /// @param startIndex initial value to startListening the iteration from. This is passed as pointer and
...
public:
    ...
    /// @param startIndex initial value to startListening the iteration from. This is passed by pointer and is left-shifted
...
enum class TimestampUnit : base::type::EnumType { Microsecond = 0, Millisecond = 1, Second = 2, Minute = 3, Hour = 4, Day = 5 };
...
public:
    ...
    static void buildBaseFilename(
        const std::string &fullPath, char buff[], std::size_t limit = base::consts::kSourceFilenameMaxLength,
...
private:
    static char *
    parseFormat(char *buf, std::size_t bufSz, const char *format, const struct tm *tInfo, std::size_t msec, const base::SubsecondPrecision *ssPrec);
};
...
public:
    ...
    /// @brief Returns size number of arguments. This exclude argv[0]
...
public:
    ...
    /// @return Iterator pointer from startListening of repository
    ...
    /// @return Constant iterator pointer from startListening of repository
...
namespace base {
/// @brief Represents log format containing flags and date format. This is used internally to startListening initial log
    ...
    Conf_T unsafeGetConfigByVal(Level level, const std::unordered_map<Level, Conf_T> *confMap, const char *confName) {
        ...
        if (it == confMap->end()) {
            try {
                ...
                ELPP_INTERNAL_ERROR(
                    "Unable to get configuration [" << confName << "] for level [" << LevelHelper::convertToString(level) << "]" << std::endl
...
public:
    ...
    public:
        ...
        inline bool operator()(const HitCounter *counter) {
            return ((counter != nullptr) && (strcmp(counter->m_filename, m_filename) == 0) && (counter->m_lineNumber == m_lineNumber));
        }
...
public:
    LogDispatchData()
        ...
        , m_dispatchAction(base::DispatchAction::None) {}
...
public:
    LogBuilder()
        : m_termSupportsColor(base::utils::OS::termSupportsColor()) {}
...
public:
    LogMessage(
        Level level, const std::string &file, base::type::LineNumber line, const std::string &func, base::type::VerboseLevel verboseLevel,
        Logger *logger)
        : m_level(level)
        , m_file(file)
        , m_line(line)
...
public:
    MessageBuilder(void)
        : m_logger(nullptr)
        , m_containerLogSeparator(ELPP_LITERAL("")) {}
...
#define ELPP_ITERATOR_CONTAINER_LOG_FIVE_ARG(temp)                                                                                                   \
    ...
    /// @param ElementInstance Instance of element to be fed out. eventQueue name is "elem". See WXELPP_ENABLED macro
...
public:
    Writer(
        Level level, const char *file, base::type::LineNumber line, const char *func,
        base::DispatchAction dispatchAction = base::DispatchAction::NormalLog, base::type::VerboseLevel verboseLevel = 0)
        : m_msg(nullptr)
        , m_level(level)
        , m_file(file)
        , m_line(line)
...
void Logger::log_(Level level, int vlevel, const T &log) {
    if (level == Level::Verbose) {
        if (ELPP->vRegistry()->allowed(vlevel, __FILE__)) {
            base::Writer(Level::Verbose, "FILE", 0, "FUNCTION", base::DispatchAction::NormalLog, vlevel).construct(this, false) << log;
        } else {
...
public:
    PerformanceTracker(
        ...
        const std::string &loggerId = std::string(el::base::consts::kPerformanceLoggerId), bool scopedLog = true,
    ...
    void
    checkpoint(const std::string &id = std::string(), const char *file = __FILE__, base::type::LineNumber line = __LINE__, const char *func = "");
    inline Level level(void) const { return m_level; }
...
protected:
    void handle(const PerformanceTrackingData *data) {
        ...
        if (m_data->dataType() == PerformanceTrackingData::DataType::Complete) {
            ss << ELPP_LITERAL("Executed [") << m_data->blockName()->c_str() << ELPP_LITERAL("] in [") << *m_data->formattedTimeTaken()
               << ELPP_LITERAL("]");
        } else {
...
public:
    ...
    static void
    logCrashReason(int sig, bool stackTraceIfAvailable = false, Level level = Level::Fatal, const char *logger = base::consts::kDefaultLoggerId);
#endif // defined(ELPP_FEATURE_ALL) || defined(ELPP_FEATURE_CRASH_LOG)
    ...
    /// @brief Installs post log startListening callback, this callback is triggered when log is dispatched
    template <typename T>
    static inline bool installLogDispatchCallback(const std::string &id) {
        return ELPP->installLogDispatchCallback<T>(id);
    }
    /// @brief Uninstalls log startListening callback
    template <typename T>
...
#define TIMED_SCOPE_IF(obj, blockname, condition)                                                                                                    \
    el::base::type::PerformanceTrackerPtr obj(condition ? new el::base::PerformanceTracker(blockname, ELPP_MIN_UNIT) : nullptr)
...
#define TIMED_BLOCK(obj, blockName)                                                                                                                  \
    for (struct {                                                                                                                                    \
         ...
         = { 0, el::base::type::PerformanceTrackerPtr(new el::base::PerformanceTracker(blockName, ELPP_MIN_UNIT)) };                                 \
         obj.i < 1; ++obj.i)
...
#define CVERBOSE(writer, vlevel, dispatchAction, ...)                                                                                                \
    ...
    writer(el::Level::Verbose, __FILE__, __LINE__, ELPP_FUNC, dispatchAction, vlevel).construct(el_getVALength(__VA_ARGS__), __VA_ARGS__)
#else
...
#define CCHECK_STREQ(str1, str2, ...)                                                                                                                \
    CLOG_IF(!el::base::utils::Str::cStringEq(str1, str2), FATAL, __VA_ARGS__) << "Check failed: [" << #str1 << " == " << #str2 << "] "
#define CCHECK_STRNE(str1, str2, ...)                                                                                                                \
    CLOG_IF(el::base::utils::Str::cStringEq(str1, str2), FATAL, __VA_ARGS__) << "Check failed: [" << #str1 << " != " << #str2 << "] "
#define CCHECK_STRCASEEQ(str1, str2, ...)                                                                                                            \
    CLOG_IF(!el::base::utils::Str::cStringCaseEq(str1, str2), FATAL, __VA_ARGS__) << "Check failed: [" << #str1 << " == " << #str2 << "] "
